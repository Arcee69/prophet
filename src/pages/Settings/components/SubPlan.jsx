...
const SubPlan = ({ handleClose }) => {
  ...
  const subscriptionPlans = subSettings?.data || [];
  const ondemandPlans = subSettings?.ondemandPlans || [];
  ...
  return (
    <div className="bg-white w-full max-w-2xl mx-auto mt-8 h-[600px] rounded-lg shadow-lg overflow-hidden">
      <div className="p-6">
        ...
        >
          {({ values, setFieldValue, isSubmitting }) => {
            // Calculate amount whenever relevant values change
            useEffect(() => {
              if (values.type === 'subscription' && values.subscription_plan_id && values.subscription_duration) {
                ...
                if (selectedPlan) {
                  const monthlyAmount = selectedPlan.monthly_amount;
                ...
                if (selectedPlan) {
                  setFieldValue('amount', selectedPlan.price.toFixed(2));
                }
              } else {
                setFieldValue('amount', '');
              }
            }, [values.subscription_plan_id, values.subscription_duration, values.ondemand_plan_id, values.type]);
